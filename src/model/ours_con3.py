...
class RCAB(nn.Module):
    def __init__(
        ...
        self.dcn = DCN.DCN_layer(n_feat, n_feat, kernel_size, padding=(kernel_size - 1) // 2, bias= False)
        self.conv_1 = conv(n_feat, n_feat, 1)

        self.conv_2 = conv(n_feat, n_feat, 1)
    ...
    def forward(self, x):
        ...
        # import pdb
        # pdb.set_trace()
        scale = self.conv_1(x[2])
        shift = self.conv_2(x[2])
        out = x[0]*scale+shift
        # branch 2
        dcn_out = self.dcn(x[0], x[2])+x[0]

        out = out + self.ca(x) + dcn_out
        # out = out + x[0]
